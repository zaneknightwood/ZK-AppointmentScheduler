...
/**
 * This class controls the application window for the ModifyAppointment.fxml screen.
 */
...
public class ModifyAppointment implements Initializable {
    ...
    private Appointment apptToModify = null;
     ...
     * appointment information from the Appointment View controller. It sets all fields to the information contained in the
     * collected appointment. It also collects the user information from the Appointment View controller.
     */
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        apptToModify = AppointmentView.getApptToModify();
        ...
        appID.setText(String.valueOf(apptToModify.getAppointID()));
        description.setText(apptToModify.getDescription());
        title.setText(apptToModify.getTitle());
        loc.setText(apptToModify.getLocation());
        contact.setItems(allContacts);
        contact.setValue(Contact.contactNameFromAppointment(apptToModify, allContacts));
        type.setText(apptToModify.getType());
        startDate.setValue(apptToModify.getStartDateTime().toLocalDate());
        endDate.setValue(apptToModify.getEndDateTime().toLocalDate());
        while (start.isBefore(end.plusSeconds(1))) {
            startTime.getItems().add(start);
            endTime.getItems().add(start);
            start = start.plusMinutes(15);
        }
        startTime.getSelectionModel().select(apptToModify.getStartDateTime().toLocalTime());
        endTime.getSelectionModel().select(apptToModify.getEndDateTime().toLocalTime());
        userID.setItems(allUsers);
        userID.setValue(User.userNameFromAppt(apptToModify, allUsers));
        customerID.setItems(allCustomers);
        customerID.setValue(Customer.getCustFromAppt(apptToModify, allCustomers));
    }
     ...
     * It then updates the collected appointment in the database based on the appointmentID and the collected information. Error checking
     * is in place to ensure the appointment is complete, within the business operating hours of 8am - 10pm EST Monday - Friday, and does
     * not conflict with any other appointment for the specified customer. It then alerts the user that the appointment has been saved
     * and returns to Appointments View.
     *
     * @param actionEvent The "Save" button click on the Modify Appointment screen.
     */
    public void onSave(ActionEvent actionEvent) {
        try {
            ...
            LocalDateTime st = LocalDateTime.of(startDate.getValue(), startTime.getSelectionModel().getSelectedItem());
            LocalDateTime en = LocalDateTime.of(endDate.getValue(), endTime.getSelectionModel().getSelectedItem());
            ...
            LocalDateTime lastUp = LocalDateTime.now();
            String lastB = thisUser.getUserName();
            boolean isInsideHours = DateAndTimeProcessing.checkAppointmentInsideHours(st, en);

            if(isInsideHours) {
                Appointment newAppt = new Appointment(apptToModify.getAppointID(), t, d, l, ty, st, en, apptToModify.getCreateDate(), apptToModify.getCreatedBy(), lastUp, lastB, cuID, usID, coID);
                Appointment conflictApp = Appointment.appointmentOverlap(newAppt, allAppointments);
                if (conflictApp == null) {
                    Updates.updateAppointment(newAppt);
     ...
     *
     * @param actionEvent The "Cancel" button click on the Modify Appointment screen.
    ...
    public void onCancel(ActionEvent actionEvent) throws IOException {
        ...
        alert.setHeaderText("Appointment changes have not been saved!");
     ...
     *
     * @param actionEvent The "Start Time" selection click on the Modify Appointment screen.
     */
    public void onStartTime(ActionEvent actionEvent) {
        LocalTime start = startTime.getValue();
        endTime.getSelectionModel().select(start.plusMinutes(30));
    }

    /**
     * The onEndTime method checks if the start time is after the end time and throws an error if it is.
     *
     * @param actionEvent The "End Time" selection click on the Modify Appointment screen.
     */
     ...
     *
     * @param actionEvent The "Start Date" selection click on the Modify Appointment screen.
     */
     ...
     *
     * @param actionEvent The "End Date" selection click on the Modify Appointment Screen.
     */
